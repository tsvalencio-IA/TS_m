 ...
 * Mantido 100% intacto e formatado profissionalmente
 */
window.MatDefs = {
...
/**
 * ORACLE ENGINE: VALIDAÇÃO FÍSICA E GRANULAR
 */
window.OracleEngine = {
    rules: { 
    ...
    validateAndFix: (mod) => {
        ...
        // Validação de integridade das medidas customizadas das portas e gavetas
...
/**
 * BOM ENGINE: CÁLCULO FABRIL REAL (CHAPA A CHAPA)
 */
window.BOMEngine = {
    calculateModule: (mod) => {
        ...
        // Custo das Frentes Paramétricas
...
/**
 * THREE ENGINE E APP CONTROL (RENDERIZAÇÃO ESTRUTURAL 100% OCA E INTELIGENTE)
 */
window.ThreeEngine = {
    ...
    /**
     * O CORAÇÃO DO SISTEMA 10/10. 
     * Módulos ocos com faces de exposição vs faces de trabalho (Balcão Fini / Nova Romana)
     */
    rebuildScene: () => {
        ...
        window.AppState.modules.forEach(mod => {
            ...
            // SEPARAÇÃO LÓGICA DE GEOMETRIAS PURAS vs CAIXARIA
            if (mod.tipo === 'porta_avulsa') {
                ...
                criarBloco(w, h, d, 0, h/2, 0, activeMatFront);
            } 
            else if (mod.formato === 'redondo') { 
                ...
                if (mod.tipo === 'sofa') { 
                    criarBloco(w, h*0.4, d, 0, h*0.2, 0, matBase); 
                    criarBloco(esp*4, h*0.8, d, -w/2+esp*2, h*0.4, 0, matEsq); 
                    criarBloco(esp*4, h*0.8, d, w/2-esp*2, h*0.4, 0, matDir); 
                    criarBloco(w, h*0.6, esp*4, 0, h*0.7, -d/2+esp*2, matFundo); 
                } else if (mod.tipo === 'cadeira') { 
                    criarBloco(w, esp*3, d, 0, h*0.5, 0, matBase); 
                    criarBloco(0.04, h*0.5, 0.04, -w/2+0.02, h*0.25, -d/2+0.02, matEsq); 
                    criarBloco(0.04, h*0.5, 0.04, w/2-0.02, h*0.25, -d/2+0.02, matDir); 
                    criarBloco(0.04, h*0.5, 0.04, -w/2+0.02, h*0.25, d/2-0.02, matEsq); 
                    criarBloco(0.04, h*0.5, 0.04, w/2-0.02, h*0.25, d/2-0.02, matDir); 
                    criarBloco(w, h*0.5, 0.04, 0, h*0.75, -d/2+0.02, matFundo); 
                } else {
                    criarBloco(w, h, d, 0, h/2, 0, activeMatFront); 
                }
            } 
                ...
                else criarBloco(w, esp*2, d, 0, h-esp, 0, matTeto); 
                
                if (mod.formato === 'dobravel') { 
                    criarBloco(0.03, h, 0.03, -w/2+0.1, h/2, 0); 
                    criarBloco(0.03, h, 0.03, w/2-0.1, h/2, 0); 
                    ...
                    criarBloco(wL, esp*2, lenL, pxL, h-esp, pzL, mod.tampoVidro ? matVidro : matTeto); 
                    criarBloco(esp, h-esp*2, d-0.1, -w/2+esp/2, h/2-esp, 0, matEsq); 
                    criarBloco(esp, h-esp*2, d-0.1, w/2-esp/2, h/2-esp, 0, matDir);  
                    criarBloco(wL-0.1, h-esp*2, esp, pxL, h/2-esp, pzL - lenL/2 + esp/2, matFundo); 
                } else { 
                    criarBloco(esp, h-esp*2, d-0.1, -w/2+esp/2, h/2-esp, 0, matEsq); 
                    criarBloco(esp, h-esp*2, d-0.1, w/2-esp/2, h/2-esp, 0, matDir); 
                    criarBloco(w-esp*2, h/4, esp, 0, h-h/8-esp*2, 0, matFundo); 
                } 
            } 
            else if (mod.tipo === 'painel_tv') { 
                criarBloco(w, h, esp, 0, h/2, -d/2+esp/2, matFundo); 
                drawRipado(w, h, 0, h/2, -d/2+esp); 
                if (mod.gavetas || mod.portas) { 
                    const rH = h*0.25, rY = rH/2 + 0.1; 
                    criarBloco(w, esp, d, 0, rY-rH/2, 0, matBase); 
                    criarBloco(w, esp, d, 0, rY+rH/2, 0, matTeto); 
                    criarBloco(esp, rH, d, -w/2+esp/2, rY, 0, matEsq); 
                    criarBloco(esp, rH, d, w/2-esp/2, rY, 0, matDir); 
                ...
                // =========================================================================
                // LÓGICA MESTRE PARA CAIXARIA OCA E PRATELEIRAS EXTERNAS (ESTILO FINI)
                // Totalmente detalhada e protegida contra placebos
                // =========================================================================
                const isBalcao = (mod.tipo === 'balcao'); 
                const prEx = mod.prateleirasExternas > 0 ? 0.20 : 0; 
                const chassiD = d - prEx; 
                
                // Mapeamento dos Lados (Face Cliente vs Face Funcionário)
                let faceTrabalhoZ = chassiD / 2; // Z Positivo: Área do trabalhador
                let faceAtendimentoZ = -chassiD / 2; // Z Negativo: Painel Fundo e Área do cliente
                let dirTrabalho = 1; 
                let dirAtendimento = -1;

                // Se o usuário pedir para inverter (Frentes Internas no L)
                ...
                // Balcões comerciais por padrão inverteriam, mas mantemos o controle manual rígido
                if (isBalcao) {
                ...
                // 1. ESQUELETO (Teto, Base e Laterais Ocas)
                const zCentroEsqueleto = (faceTrabalhoZ + faceAtendimentoZ) / 2;
                
                // Base
                criarBloco(w, esp, chassiD, 0, esp/2, zCentroEsqueleto, matBase); 
                // Teto/Tampo
                if (mod.tampoVidro) criarBloco(w, esp, chassiD, 0, h-esp/2, zCentroEsqueleto, matVidro); 
                else criarBloco(w, esp, chassiD, 0, h-esp/2, zCentroEsqueleto, matTeto); 
                // Laterais
                criarBloco(esp, h-esp*2, chassiD, -w/2+esp/2, h/2, zCentroEsqueleto, matEsq); 
                criarBloco(esp, h-esp*2, chassiD, w/2-esp/2, h/2, zCentroEsqueleto, matDir); 

                // 2. PAINEL DE FUNDO (Bloqueio do Cliente, deixa a Face do Trabalhador 100% OCA)
                criarBloco(w-esp*2, h-esp*2, 0.006, 0, h/2, faceAtendimentoZ - (0.003 * dirAtendimento), matFundo);
                
                // 3. PRATELEIRAS EXTERNAS E RIPADO (Para Exposição/Cliente)
                if (prEx > 0 || mod.ripadoFrontal) { 
                    // Fundo falso de ancoragem do ripado/prateleiras externas
                    criarBloco(w-esp*2, h-esp*2, esp, 0, h/2, faceAtendimentoZ + (esp/2 * dirAtendimento), activeMatFront); 
                    ...
                    if (prEx > 0) { 
                        ...
                        for(let i=1; i<=mod.prateleirasExternas; i++) {
                            // Prateleiras do lado do cliente
                            criarBloco(w-esp*2, esp, prEx, 0, esp+epEx*i, faceAtendimentoZ + (prEx/2 * dirAtendimento)); 
                        }
                    } 
                }
                
                // 4. MÓDULO EM L (Braço de Retorno)
                if ((mod.formato === 'L_esq' || mod.formato === 'L_dir') && rL > d) { 
                    const isE = mod.formato === 'L_esq'; 
                    const wL = chassiD, lenL = rL - d; 
                    
                    // Posição de ancoragem do braço - Z muda dependendo de onde o funcionário está
                    ...
                    // Fechamento da ponta do L (Sólido)
                    criarBloco(wL, h-esp*2, esp, pxL, h/2, pzL + (lenL/2 * dirTrabalho) - (esp/2 * dirTrabalho), matFundo); 
                    
                    // Fechamento Lateral do L (Face da Parede/Cliente)
                ...
                // 5. PRATELEIRAS INTERNAS (Funcionário)
                if (mod.prateleiras > 0) { 
                    const epIn = (h-esp*2) / (mod.prateleiras + 1); 
                    for (let i=1; i<=mod.prateleiras; i++) {
                        criarBloco(w-esp*2, esp, chassiD-0.02, 0, esp+epIn*i, zCentroEsqueleto); 
                    }
                }
                ...
                // 6. FRENTES INTERNAS (Portas e Gavetas na Face do Trabalhador)
            ...
            // Aplica posições finais na cena global
        ...
        processPhoto: (input) => { 
            if (input.files && input.files[0]) { 
                ...
                r.onload = () => { 
                    ...
                    window.App.ui.toast("Imagem carregada para Visão."); 
                }; 
                r.readAsDataURL(f); 
            } 
        },
