/**
 * ESTADO GLOBAL E CATÁLOGO DE MATERIAIS EXPANDIDO
 */
window.MatDefs = {
...
/**
 * ORACLE ENGINE: VALIDAÇÃO FÍSICA APRIMORADA
 */
window.OracleEngine = {
    ...
    validateAndFix: (mod) => {
        ...
        if (mod.medidasCustomizadas) {
            ...
            if (totalW_Portas > mod.largura) fixes.push({k:'medidasCustomizadas', v:{}, msg:'Atenção! A soma das larguras das portas ultrapassa o vão. Medidas individuais resetadas.'});
            if (exceededHeight) fixes.push({k:'medidasCustomizadas', v:{}, msg:'Atenção! Uma peça é mais alta que a caixaria. Medidas individuais resetadas.'});
        }
...
/**
 * BOM ENGINE: CÁLCULO FABRIL REAL COM GRANULARIDADE (CHAPA A CHAPA)
 */
window.BOMEngine = {
    calculateModule: (mod) => {
        ...
        if (mod.tipo === 'porta_avulsa') { 
            let a = w*h; areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, mod.materiaisCustomizados?.frentes?.['porta_avulsa_0']);
            ...
            let a = (w*0.3 + d*0.3*2 + w*d); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, mod.materiaisCustomizados?.frentes?.['gaveta_avulsa_0']);
            qtdCorredicas += 1; hwCusto += 25.0; 
        } 
        else if (mod.tipo === 'painel_fixo') { 
            let a = (w*h); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null);
        } 
        else if (mod.tipo === 'sofa' || mod.tipo === 'puff' || mod.tipo === 'cadeira') { 
            let a = (w * h * d) * 2; areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); laborMult = 1.5; 
        } 
        else if (mod.tipo === 'mesa' && !mod.formato.includes('L_')) { 
            let aTeto = (w * d); areaChassiGlobal += aTeto; totalCostOfModuleMaterials += calcCustoPeca(aTeto, mod.materiaisCustomizados?.estrutura?.['teto']);
            let aPernas = (h * 0.6) * 2; areaChassiGlobal += aPernas; totalCostOfModuleMaterials += calcCustoPeca(aPernas, mod.materiaisCustomizados?.estrutura?.['lateralEsq']);
            if(mod.formato === 'dobravel') { hwCusto += 150; laborMult = 1.2; } 
        } 
        else if (mod.formato === 'redondo') { 
            let a = (Math.PI * Math.pow(w / 2, 2)) * 2 + (Math.PI * w * h); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); laborMult = 1.8; 
        } 
        else if (mod.formato === 'triangular') { 
            let a = (w * d * h); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); laborMult = 1.4; 
        } 
        else {
            let aLat = (h * d * 2); areaChassiGlobal += aLat; 
            totalCostOfModuleMaterials += calcCustoPeca(aLat/2, mod.materiaisCustomizados?.estrutura?.['lateralEsq']) + calcCustoPeca(aLat/2, mod.materiaisCustomizados?.estrutura?.['lateralDir']);
            
            let aTetoBase = (w * d * 2); areaChassiGlobal += aTetoBase; 
            totalCostOfModuleMaterials += calcCustoPeca(aTetoBase/2, mod.materiaisCustomizados?.estrutura?.['teto']) + calcCustoPeca(aTetoBase/2, mod.materiaisCustomizados?.estrutura?.['base']);
            
            let aFundo = (w * h); areaChassiGlobal += aFundo; 
            totalCostOfModuleMaterials += calcCustoPeca(aFundo, mod.materiaisCustomizados?.estrutura?.['fundo']);
            
            if (mod.formato.includes('L_')) { const rL = (mod.retornoL / 1000) - d; if (rL > 0) { let aL = (h * rL * 2) + (d * rL * 2) + (d * h); areaChassiGlobal += aL; totalCostOfModuleMaterials += calcCustoPeca(aL, null); } } 
            else if (mod.formato.includes('canto_')) { let aC = (w * h * 2) + (w * d); areaChassiGlobal += aC; totalCostOfModuleMaterials += calcCustoPeca(aC, null); laborMult = 1.3; }
        }

        if (mod.prateleiras) { let a = mod.prateleiras * (w * d); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); }
        if (mod.prateleirasExternas) { let a = mod.prateleirasExternas * (w * 0.20); areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); } 
        ...
        if (mod.tipo !== 'porta_avulsa' && mod.tipo !== 'gaveta_avulsa' && mod.tipo !== 'painel_fixo') {
            if (mod.gavetas > 0) { 
                for(let i=0; i<mod.gavetas; i++) {
                    ...
                    let a = (cG_W*0.3 + d*0.3*2 + cG_W*d); areaChassiGlobal += a;
            ...
            if (mod.portas > 0) { 
                ...
                for(let i=0; i<portasReais; i++) {
                    ...
                    let a = (cP_W * cP_H); areaChassiGlobal += a;
                    ...
                    else { let dobradicasPorPorta = cP_H > 1.5 ? 4 : (cP_H > 1.0 ? 3 : 2); qtdDobradicas += dobradicasPorPorta; hwCusto += dobradicasPorPorta * 7.5; }
                }
            }
        }

        if (mod.ripadoFrontal) { let a = (w * h) * 1.5; areaChassiGlobal += a; totalCostOfModuleMaterials += calcCustoPeca(a, null); laborMult += 0.3; }
        if (mod.tampoVidro) hwCusto += (w * d) * 150; 

        const areaTotalReal = areaChassiGlobal * 1.15;
        const matCustoFinal = totalCostOfModuleMaterials * 1.15;

        return { area: areaTotalReal, hardwareCost: hwCusto, laborMult: laborMult, customMatCostTotal: matCustoFinal, counts: { dobradicas: qtdDobradicas, corredicas: qtdCorredicas, trilhos: qtdTrilhos, pistoes: qtdPistoes } }; 
    },
    ...
    update: () => {
        ...
        window.AppState.modules.forEach(m => { 
            ...
            tArea += b.area; tHWDyn += b.hardwareCost; tLaborDyn += (b.area * window.AppState.costs.labor * b.laborMult); totalMatsCost += b.customMatCostTotal; 
...
/**
 * THREE ENGINE E APP CONTROL (RENDERIZAÇÃO CORRIGIDA PARA "FRENTES INTERNAS")
 */
window.ThreeEngine = {
        ...
        if (window.AppState.tool === 'add_comp') {
            if (Math.hypot(cX - window.ThreeEngine.pDown.x, cY - window.ThreeEngine.pDown.y) < 15) {
                ...
                if (hits.length > 0) {
                    ...
                    window.AppState.modules.push(mod); window.App.modules.select(novoId); window.App.modules.refreshAll();
    ...
    rebuildScene: () => {
        ...
        window.AppState.modules.forEach(mod => {
            ...
            const fillStorageArea = (boxW, boxH, boxD, boxX, boxY, boxZ, portas, gavetas, layout, abertura, dobrLado, zDir, parentGroup) => {
                ...
                if (gavetas > 0 && layout !== 'apenas_portas') {
                    ...
                    for (let i = 0; i < gavetas; i++) { 
                        ...
                        drawInteractiveFronte(parentGroup, true, cG_W, cG_H, boxD, sxG + cG_W/2, currentY + cG_H/2, gZFinal, matGaveta, '', false, zDir, 0, gavKey); currentY += cG_H;
                    }
                }
                ...
                if (portas > 0 && layout !== 'apenas_gavetas') {
                    ...
                    for (let i = 0; i < portas; i++) {
                        ...
                        drawInteractiveFronte(parentGroup, false, cP_W, cP_H, boxD, currentX + cP_W/2, syP + cP_H/2, doorZReal, matPorta, abertura, isLeft, zDir, tX, portKey); currentX += cP_W;
                    }
                }
            };
            ...
            if (mod.tipo === 'porta_avulsa') drawInteractiveFronte(g, false, w, h, d, 0, h/2, 0, activeMatFront, mod.abertura, mod.dobradicaLado === 'esq', 1, w, 'porta_avulsa_0');
                ...
                // LÓGICA 100% CORRIGIDA DE INVERSÃO DO PAINEL TRASEIRO E LATERAIS DO "L"
                const isBalcao = (mod.tipo === 'balcao'); const prEx = mod.prateleirasExternas > 0 ? 0.20 : 0; const chassiD = d - prEx; 
                
                const inverteZ = mod.frentesInternas ? -1 : 1; 
                const zDir = (isBalcao ? -1 : 1); 
                const fZ = (chassiD/2 * zDir) + (esp/2 * zDir); 
                const bZ = (chassiD/2 * -zDir) + (esp/2 * -zDir); 

                const fZ_storage = mod.frentesInternas ? bZ : fZ;
                const fundoZ_main = mod.frentesInternas ? fZ : bZ;
                const zDir_storage = zDir * inverteZ;
                ...
                // Fundo do módulo principal agora é ejetado dinamicamente para o lado oposto das portas
                criarBloco(w-esp*2, h-esp*2, 0.006, 0, h/2, fundoZ_main - 0.003 * Math.sign(fundoZ_main), matFundo);
                ...
                if ((mod.formato === 'L_esq' || mod.formato === 'L_dir') && rL > d) { 
                    const isE = mod.formato === 'L_esq'; const wL = d, lenL = rL - d; const pxL = isE ? -w/2+wL/2 : w/2-wL/2; const pzL = isBalcao ? d/2 + lenL/2 : -d/2 - lenL/2; 
                    
                    criarBloco(wL, esp, lenL, pxL, esp/2, pzL, matBase); 
                    criarBloco(wL, esp, lenL, pxL, h-esp/2, pzL, mod.tampoVidro?matVidro:matTeto); 
                    // Painel da extremidade (ponta do L)
                    criarBloco(wL, h-esp*2, esp, pxL, h/2, pzL - (lenL/2*zDir) + (esp/2*zDir), matFundo); 
                    
                    // Cálculo dinâmico para garantir que o painel da parede não bloqueie a frente do móvel!
                    ...
                    criarBloco(esp, h-esp*2, lenL, painelX, h/2, pzL, isE?matEsq:matDir); 

                    const returnGroup = new THREE.Group(); returnGroup.position.set(pxL, h/2, pzL); returnGroup.rotation.y = isE ? -Math.PI/2 : Math.PI/2; 
                    
                    let fZ_return = (isBalcao ? -chassiD/2 : chassiD/2);
                    let zDir_return = isBalcao ? -1 : 1;
                    if (mod.frentesInternas) { fZ_return = -fZ_return; zDir_return = -zDir_return; }

                    fillStorageArea(lenL-esp*2, h-esp*2, chassiD, 0, 0, fZ_return, mod.portas, mod.gavetas, mod.layoutInterno, mod.abertura, mod.dobradicaLado, zDir_return, returnGroup); 
                    g.add(returnGroup); 
                }
...
window.App = {
    ...
    modules: {
        ...
        removeCurrent: () => {
            ...
            window.AppState.modules = window.AppState.modules.filter(m => m.id !== idToRem); window.App.modules.select(null); window.App.ui.fecharEditor(); window.App.modules.refreshAll(); window.App.ui.toast("Módulo removido.", "success");
        },
        ...
        exportarProjeto: () => {
            ...
            const downloadAnchorNode = document.createElement('a'); downloadAnchorNode.setAttribute("href", dataStr); downloadAnchorNode.setAttribute("download", "projeto.json");
    ...
    ui: {
        ...
        openLiveEditor: (id) => {
            ...
            if (custContainer) {
                ...
                const addField = (key, label) => {
                    ...
                    let matOpts = `<option value="">-- Cor Padrão --</option>`; Object.keys(window.MatDefs).forEach(k => { matOpts += `<option value="${k}" ${mat===k?'selected':''}>${window.MatDefs[k].label}</option>`; });
                ...
                parts.forEach((p, idx) => {
                    const pMat = m.materiaisCustomizados?.estrutura?.[p] || ''; let popts = `<option value="">-- Cor Padrão --</option>`; Object.keys(window.MatDefs).forEach(k => popts += `<option value="${k}" ${pMat===k?'selected':''}>${window.MatDefs[k].label}</option>`);
        ...
        syncCustomStruct: (part) => {
            const elId = document.getElementById('eId'); if(!elId) return; const m = window.AppState.modules.find(x => x.id === elId.value); if(!m) return;
            if(!m.materiaisCustomizados) m.materiaisCustomizados = { frentes:{}, estrutura:{} }; const mat = document.getElementById(`custE_${part}`).value;
    ...
    ar: {
        ...
        toggleAR: () => { 
            ...
            if (window.AppState.arActive) { if(window.AppState.imagemFundoURL) { container.style.backgroundImage = `url(${window.AppState.imagemFundoURL})`; container.style.backgroundSize = 'cover'; container.style.backgroundPosition = 'center'; container.style.backgroundRepeat = 'no-repeat'; } else { container.style.background = 'transparent'; } if(txt) txt.innerText = 'Desativar Fundo Real'; } 
    ...
    ai: {
        toggleRecording: async () => {
            ...
            if(window.AppState.mediaRecorder && window.AppState.mediaRecorder.state !== "inactive") { window.AppState.mediaRecorder.stop(); btn.classList.remove('recording'); btn.innerHTML = "<i class='fas fa-microphone'></i> Gravar Voz"; } 
